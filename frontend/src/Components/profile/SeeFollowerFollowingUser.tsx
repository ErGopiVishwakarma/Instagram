...
import { CommentType, LikeType } from '../../Types/otherType';
...
const SeeFollowerFollowingUser = ({text,el}:HandleFun) => {
  ...
  return (
    ...
    <Dialog open={open} handler={handleOpenFun} size='xs' style={{border:'none',outline:'none'}}>
      ...
      <DialogBody
        ...
        {el?.length > 0 ? (
          el?.map((ele: LikeType, ind: number) => {
            return (
...
export default memo(SeeFollowerFollowingUser)